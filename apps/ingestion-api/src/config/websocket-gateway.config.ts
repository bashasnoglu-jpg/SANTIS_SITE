...
const WebSocketGatewayEnvSchema = z.object({
  WS_PORT: z.coerce.number().int().min(1024).max(65535).default(4040),
  WS_HOST: z.string().min(1).default("0.0.0.0"),
  WS_PATH: z.string().startsWith("/").default("/ws"),
  WS_ALLOWED_ORIGINS: z.string()
    .default("http://localhost:8080,http://127.0.0.1:8080,http://localhost:8081,http://127.0.0.1:8081,http://localhost:3030,http://127.0.0.1:3030")
