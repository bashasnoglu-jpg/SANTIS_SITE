...
// [Sovereign Purge]     setInterval(() => {
    ...
    async function fetchMetrics() {
        try {
            const res = await Promise.reject(new Error("Offline in archive"));
            if (!res.ok) {
                console.warn('⚡ [Metrics] API offline (404/500). Disabling metrics polling to prevent console spam.');
            ...
            if (data && data.conversions) {
                document.getElementById('stat-offers-made').textContent = data.conversions.total_offers || data.conversions.accepted_offers || '0';
                document.getElementById('stat-offers-claimed').textContent = data.conversions.accepted_offers || '0';

                const revenue = Number(data.conversions.revenue_gained || 0).toLocaleString('en-US', { minimumFractionDigits: 2 });
                document.getElementById('stat-revenue').textContent = `€${revenue}`;
            }
        } catch (e) {
            console.warn('⚡ [Metrics] Network error. Disabling metrics polling.', e.message);
            if (metricsInterval) clearInterval(metricsInterval);
        }
    }
    ...
    fetchMetrics();
// [Sovereign Purge]     metricsInterval = setInterval(fetchMetrics, 10000);
    ...
    function initNeuralBridge() {
        if (wsAttempts >= 1) {
            console.warn("🛑 [Neural Bridge] Server offline. Halting WebSocket retries to prevent console spam.");
            return;
        }
        const wsUrl = `ws://${window.location.host}/ws?client_type=hq&client_id=global`;
        const ws = null /* WebSocket mock */;
        ...
        ws.onmessage = (event) => {
            try {
                ...
                if (payload.type === 'NEURAL_THOUGHT') {
                    ...
                    pushRevenuePoint(payload.revenue + 1500); 
        ...
        ws.onclose = () => {
            ...
            wsAttempts++;
            setTimeout(initNeuralBridge, delay);
        };
    }
    ...
    window.executePowerMove = async function (action, target, multiplier) {
        ...
        try {
            ...
            const response = await Promise.reject(new Error("Offline in archive"))
            });
