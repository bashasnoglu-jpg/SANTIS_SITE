            ...
            if (stats.country_distribution) {
                Object.entries(stats.country_distribution).forEach(function (entry) {
                    ...
                    tr.innerHTML =
                        ...
                        '<td class="text-[#d4af37]" style="padding:5px;">' + count + '</td>' +
    ...
    function renderRedirectTable() {
        ...
        items.forEach(function (r) {
            ...
            html += '<tr style="border-bottom:1px solid rgba(255,255,255,0.04);">' +
                '<td class="text-center" style="padding:8px 10px;"><span style="display:inline-block; padding:1px 6px; border-radius:4px; background:rgba(255,255,255,0.04); border:1px solid ' + langColor + '; font-size:10px; color:' + langColor + ';">' + flag + ' ' + langLabel + '</span></td>' +
                ...
                '<td class="text-center" style="padding:8px 10px;"><span style="display:inline-block; padding:1px 8px; border-radius:999px; background:rgba(255,255,255,0.05); color:' + typeColor + '; font-size:11px; font-weight:600;">' + (r.type || 301) + '</span></td>' +
                '<td class="text-center" style="padding:8px 10px;"><button class="btn-os" style="padding:3px 8px; font-size:10px; border-color:#ff4444; color:#ff4444;" data-action="redirect-delete-entry" data-from="' + escapeAttr(r.from || '') + '" data-to="' + escapeAttr(r.to || '') + '">✖</button></td>' +
                '</tr>';
        });
        ...
        try {
            ...
            if (matrix && data.lang_matrix) {
                ...
                langs.forEach(function (lang) {
                    ...
                    html += '<div class="text-center" style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:12px; padding:12px;">';
                    html += '<div style="font-size:18px; font-weight:700; color:' + color + ';">' + lang.toUpperCase() + '</div>';
                    html += '<div class="text-white" style="font-size:24px; font-weight:700; margin:8px 0;">' + lm.total + '</div>';
            ...
            if (tbody && data.violations) {
                if (data.violations.length === 0) {
                    tbody.innerHTML = '<tr><td class="text-center" colspan="4" style="color:#00ff88;">✅ İhlal bulunamadı</td></tr>';
                } else {
                    var rows = '';
                    ...
                    if (data.violations.length > 30) {
                        rows += '<tr><td class="text-center" colspan="4" style="color:#888;">... ve ' + (data.violations.length - 30) + ' daha fazla</td></tr>';
                    }
