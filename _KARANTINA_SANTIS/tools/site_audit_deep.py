...
def generate_report(root_dir, output_report):
    # Set up derived paths based on root
    root_path = Path(root_dir)
    tr_dir = root_path / "tr"
    guest_zen_dir = root_path / "guest-zen"
    ...
    if tr_dir.exists():
        for root, _, files in os.walk(str(tr_dir)):
            for f in files:
                if f.endswith('.html'):
    ...
    zen_index = guest_zen_dir / 'index.html'
    if zen_index.exists():
    ...
    dashboard_js = root_path / "admin" / "dashboard.js"
    if dashboard_js.exists():
        with open(dashboard_js, 'r', encoding='utf-8') as f:
            if 'websocket' in f.read().lower() or 'bridgesocket' in f.read().lower():
        ...
        report_lines.append("⚠️ **[Medium]** Dashboard script dosyası bulunamadı, kontrol atlandı.")
    ...
    print(f"Deep Scan Complete. Artifact generated at {output_path.resolve()}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="SANTIS MASTER OS - DEEP SCAN AUDIT")
    parser.add_argument(
        "--root", 
        type=str, 
        default=str(Path(__file__).resolve().parents[1]),
        help="Root directory of the SANTIS_SITE repository"
    )
    parser.add_argument(
        "--output", 
        type=str, 
        default=str(Path(__file__).resolve().parents[1] / "reports" / "SANTIS_DEEP_SCAN_REPORT.md"),
        help="Output path for the generated markdown report"
    )
    args = parser.parse_args()

    # Pass configuration to generate_report
