...
import { FileBackedAggregatedOptimizerMemory } from './server/core/experiments/optimizer/optimizer.memory.aggregate.file.ts';
import { TemporalAggregatedOptimizerAdapter } from './server/core/experiments/optimizer/optimizer.temporal.adapter.ts';
...
run().catch((error) => {
  console.error('❌ V2.5 TEMPORAL AGGREGATED OPTIMIZER SMOKE TEST FAILED');
  console.error(error);
  process.exitCode = 1;
});
