...
export class SovereignSurgePricing extends HTMLElement {
    ...
    updatePricing(surge) {
        if (surge.isEngaged && surge.multiplier > 1) {
            ...
            // 1. Nefes Alan Altın Hale Animasyonu (token class'ları ile)
            this.haloWrapper.classList.add('shadow-nv-gold-glow', 'border-nv-gold');
            ...
            this.currentPriceEl.innerHTML = `
                ...
                <span class="text-nv-gold font-bold text-3xl transition-all duration-700 animate-pulse">€${newPrice}</span>
            `;

            // 3. Sessiz Lüks Metni
            this.surgeIndicator.innerHTML = `
                <span class="text-nv-gold text-xs uppercase tracking-widest flex items-center justify-center gap-1 mt-2">
    ...
    renderSkeleton() {
        this.innerHTML = `
            <div id="halo-wrapper" class="relative bg-gray-900 border border-gray-700 p-6 rounded-lg transition-all duration-1000 ease-in-out w-72 text-center overflow-hidden mx-auto mt-8">
                ...
                <div id="price-container" class="min-h-10 flex items-center justify-center">
