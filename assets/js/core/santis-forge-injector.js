...
export class SantisDiffEngine {
    static patch(container, virtualSack) {
        ...
        for (const liveCard of container.children) {
            if (liveCard.classList.contains('santis-rail-card')) {
...
export class SovereignForgeInjector {
    ...
    static forgeCardHTML(ritualDTO) {
        ...
        return `
        <div class="santis-rail-card ritual-card ${signatureClass} group relative w-[85vw] md:w-[480px] shrink-0 h-[650px] rounded-3xl overflow-hidden cursor-pointer block disable-hover-decoration" data-key="${ritualDTO.id}" data-hash="${ritualDTO.hash}" style="--card-aura: ${ritualDTO.aura_color}; display: block; box-sizing: border-box; background-color: #050505; z-index: ${armorLevel}; text-decoration: none;">
            ${signatureBadge}
            <div class="skeleton-layer absolute inset-0 bg-neutral-900 animate-pulse z-0 rounded-3xl"></div>
            <img src="${ritualDTO.image_url || '/assets/img/cards/massage.webp'}" alt="${ritualDTO.alt_text || 'Santis Ritual'}" width="320" height="427" loading="lazy" decoding="async" fetchpriority="low" onload="this.classList.remove('opacity-0', 'scale-110'); this.previousElementSibling.classList.add('hidden');" class="santis-rail-card-bg absolute inset-0 w-full h-full object-cover opacity-0 scale-110 group-hover:opacity-30 group-hover:scale-105 transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] z-10">
