...
/**
 * Ana iş parçacığına (Main Thread) nefes aldırarak kullanıcı etkileşimlerine (INP) öncelik tanır.
 */
        ...
        if (depth > 0) {
            container.querySelectorAll('a, img').forEach(node => {
                ...
                if (val && !val.startsWith('http') && !val.startsWith('//') && !val.startsWith('#') && !val.startsWith('mailto:') && !val.startsWith('tel:') && !val.startsWith('javascript:')) {
                    ...
                    // If we are deep, "/assets/img" looks for tr/tr/masajlar/assets/img (wrong)
                    // It needs ../../assets/img
...
function initNavbarInteractions() {
    ...
    if (ham && menu) {
        ...
        newHam.addEventListener('click', (e) => {
            // 1. ANINDA GÖRSEL TEPKİ (INP'yi mükemmelleştiren adım)
            newHam.classList.toggle('active');
            menu.classList.toggle('active');
            
            // 2. AĞIR İŞLEMLERİ ERTELE
...
if (typeof UIOrchestrator === 'undefined') var UIOrchestrator = {
...
function startUIOrchestrator() {
    ...
    if (nav) {
        let lastScrollY = window.scrollY;
        let ticking = false;
        const scrollThreshold = 120; // Minimum scroll before hiding
