...
document.addEventListener("DOMContentLoaded", () => {
    ...
    featuredProducts.forEach((product, index) => {
        ...
        // PHASE 6: MOOD ENGINE (Randomized or Data-Driven)
        // moods: calm (gold/green), mystic (purple/blue), energetic (orange/amber)
        const moods = ['calm', 'mystic', 'energetic'];
        // Use index to rotate through moods deterministically for visual balance
        const mood = moods[index % moods.length];
        card.dataset.mood = mood;

        const imgPath = `assets/img/cards/${product.img}`;

        // Handle badge if data exists
