        ...
        async with db.begin():
            ...
            if not registry_entry:
                registry_entry = ContentRegistry(
                    ...
                    ab_split_config=payload.ab_split_config
                )
                db.add(registry_entry)
            else:
                # 3.1 IDEMPOTENCY CHECK
                if registry_entry.active_version_hash == version_hash:
                ...
                registry_entry.active_version_hash = version_hash
                registry_entry.updated_by = actor_email
...
async def get_content_timeline(region: str, slug: str, db: AsyncSession = Depends(get_db)) -> Any:
    """
    ...
    for log in logs:
        history.append(TimelineItem(
            version_hash=log.version_hash or "unknown",
            action=log.action,
            actor=log.actor,
...
async def rollback_content(
    region: str, slug: str, request: Request, payload: RollbackRequest, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)
) -> Any:
    """
    ...
    async with db.begin():
        ...
        # IDEMPOTENCY CHECK
        if registry.active_version_hash == payload.target_version_hash:
            return PublishResponse(
                status="success",
        ...
        registry.active_version_hash = payload.target_version_hash
        registry.updated_by = "admin@santis.club" # Mock current actor
...
async def soft_delete_content(
    ...
    try:
        async with db.begin():
            ...
            active_hash = registry.active_version_hash
