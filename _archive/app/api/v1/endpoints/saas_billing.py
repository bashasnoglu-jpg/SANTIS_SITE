    ...
    if stripe:
        ...
        try:
            ...
            session = stripe.checkout.Session.create(
                mode="subscription",
                payment_method_types=["card"],          # Link uyarısını kapat
...
async def stripe_webhook(
    ...
    if stripe and webhook_secret and webhook_secret.startswith("whsec_") and len(webhook_secret) > 20:
        try:
    ...
    # tenant_subscriptions tablosu alembic tarafından yönetiliyor
    ...
    if etype in ("checkout.session.completed", "payment_intent.succeeded"):
        ...
        if tenant_id:
            ...
            await db.execute(text("""
                ...
                VALUES (:tid, :plan, 'active', :sub_id, NOW())
                ON CONFLICT(tenant_id) DO UPDATE
                SET plan=EXCLUDED.plan,
                    status='active',
                    stripe_sub_id=EXCLUDED.stripe_sub_id,
                    updated_at=NOW()
            """), {"tid": str(tenant_id), "plan": plan, "sub_id": stripe_sub_id})
            # tenants tablosunu da güncelle
            await db.execute(text("""
                UPDATE tenants SET plan=:plan WHERE id::text=:tid
            """), {"plan": plan, "tid": str(tenant_id)})
            await db.commit()

    # Abonelik iptal → status canceled
...
async def increment_usage(req: UsageRequest, db: AsyncSession = Depends(_gdb)):
    # usage_counters tablosunu garantiye al
    await db.execute(text("""
    ...
    period = __import__('datetime').date.today().strftime('%Y-%m-01')
    await db.execute(text(f"""
        INSERT INTO usage_counters (tenant_id, period_start, {col})
        VALUES (:tid, :period, 1)
        ON CONFLICT (tenant_id, period_start)
        DO UPDATE SET {col} = usage_counters.{col} + 1,
                      updated_at = datetime('now')
    """), {"tid": req.tenant_id, "period": period})
    await db.commit()
    return {"status": "ok"}
