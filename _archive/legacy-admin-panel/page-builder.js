// SANTIS PAGE BUILDER ENGINE (V5) - DEBUG OVERRIDE
// Handles Block-Based Page Editing

(function () {
    console.log("🧱 PageBuilder Override Loaded");

    // Scoped variable to avoid global collision
    let currentBlocks = [];
    ...
    window.PageBuilder = {
        init: async function () {
            if (this._initialized) {
                console.warn("🧱 PageBuilder already initialized. Skipping.");
                return;
            }
            this._initialized = true;

            console.log("🧱 PageBuilder Init Triggered (Override)");
            console.log("   - Container:", document.getElementById('pb-container'));
        ...
        createBlockElement: function (block, idx) {
            ...
            // Header
            div.innerHTML = `
                <div class="flex" style="background:#222; padding:10px; justify-content:space-between; align-items:center; cursor:move;">
                    <span class="flex" style="font-weight:bold; color:var(--os-gold); gap:10px; align-items:center;">
                        <span class="text-center" style="background:#333; width:20px; height:20px; border-radius:4px; font-size:10px;">${idx + 1}</span>
        ...
        getBlockForm: function (block, idx) {
            if (block.type === 'hero') {
                return `
                    ...
                    <div class="flex" style="gap:10px;">
                         <input id="pb-img-${idx}" class="os-input" placeholder="Görsel (uploads/hero.webp)" value="${escapeHtml(block.data.image || '')}" data-pb-field="image" data-idx="${idx}">
            ...
            if (block.type === 'gallery') {
                return `
                    <div class="text-white">Galeri Görselleri (Virgülle veya JSON)</div>
                    <textarea class="os-input" rows="2" placeholder='["img1.webp", "img2.webp"]' data-pb-field="images" data-idx="${idx}">${escapeHtml(block.data.images || '')}</textarea>
                `;
            }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (document.getElementById('pb-container')) {
            // Explicitly call init if window.PageBuilder is ready
        ...
        input.onchange = async e => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    window.PageBuilder.updateData(idx, 'image', path);
    ...
    window.generateAIForBlock = async function (idx, field) {
        ...
        try {
            ...
            window.PageBuilder.updateData(idx, field, data.text);
            // Refresh UI
            window.PageBuilder.render();
