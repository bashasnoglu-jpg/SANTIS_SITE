...
(function () {
    ...
    window.renderTable = function () {
        ...
        sorted.forEach(function (p) {
            ...
            tr.innerHTML =
                '<td><img src="../assets/img/cards/' + ((p.img && p.img !== 'undefined') ? p.img : 'santis_card_products_lux.webp') + '" data-fallback-src="https://placehold.co/40x40?text=?" width="40"></td>' +
                '<td><strong>' + p.name + '</strong></td>' +
                '<td>' + (p.cat || p.category) + '</td>' +
                '<td>' + p.price + '</td>' +
                '<td>' + (p.badge ? '<span class="text-[#d4af37]">' + p.badge + '</span>' : '-') + '</td>' +
                '<td>' +
