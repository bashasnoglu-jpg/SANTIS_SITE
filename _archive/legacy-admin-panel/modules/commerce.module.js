...
(function () {
    ...
    window.renderCosmetics = function () {
        ...
        localSothys.forEach(function (p) {
            ...
            tr.innerHTML =
                '<td><img src="' + (p.image || '/assets/img/placeholder.webp') + '" width="40" style="border-radius:4px;"></td>' +
                '<td><strong>' + p.name + '</strong><br><span style="font-size:11px; color:#888;">' + (p.benefit || '') + '</span></td>' +
                '<td><span class="text-white badge" style="background:#222;">' + p.line + '</span></td>' +
                '<td>€' + p.price + '</td>' +
                '<td>' + (p.volume || '-') + '</td>' +
                '<td>' +
    ...
    window._openCommerceModal = function (context) {
        ...
        if (previewImg) previewImg.src = '/assets/img/placeholder.webp';
    ...
    window.editItem = function (context, id) {
        ...
        var previewImg = document.getElementById('preview-img');
        if (previewImg) previewImg.src = item.image || '/assets/img/placeholder.webp';
