...
class SovereignWarRoom {
    constructor() {
        // 🎯 Sovereign WS Shield v2.0 — server.js üzerinden bağlan
        const host = window.location.hostname || 'localhost';
        const port = window.location.port || '8080';
        this.wsUrl = `ws://${host}:${port}/ws?client_type=admin`;
        this.totalRevenue = 0;
        this.retryCount = 0;
        this.maxRetries = 5;
        this.initRadar();
    ...
    processIntelligence(data) {
        // 1. TELEMETRY_BEACON — Ghost/Darwinian telemetri (server forward)
        if (data.type === 'TELEMETRY_BEACON' && data.payload) {
            const p = data.payload;
            // Darwinian Fitness Raporu (30s döngü)
            if (p.type === 'darwinian_report') {
                this.renderDarwinianFitness(p);
            }
            // Darwinian Anlık Event (tıklama/hover)
            else if (p.type === 'darwinian_event') {
                this.renderDarwinianEvent(p);
            }
            // Ghost CRM Pulse (skor bazlı)
            else if (p.current_score !== undefined) {
                this.renderPulse(data);
            }
        }
        // 2. LIVE_PULSE — sunucu telemetri heartbeat
        else if (data.type === 'LIVE_PULSE') {
            this.renderPulse(data);
        }
        // 3. BLACK_ROOM_EXECUTION — Satış infazı
        else if (data.type === 'BLACK_ROOM_EXECUTION') {
            this.renderExecution(data);
        }
        // 4. SYSTEM_BOOT — bağlantı onayı
        else if (data.type === 'SYSTEM_BOOT') {
            if (window.SantisLog) window.SantisLog.info('🟢 [War Room] Sovereign Bus v2.0 Online: ' + (data.payload?.message || ''));
        }
        // 5. 👑 APEX LOCK CONFIRMED
        else if (data.type === 'APEX_LOCK_CONFIRMED') {
            this.triggerMassiveAlert('APEX LOCKED', data.payload?.variantHash || 'unknown');
            if (window.SantisLog) window.SantisLog.info('👑 [APEX] Varyant kilitlendi: ' + (data.payload?.variantHash || ''));
        }
        else if (data.type === 'APEX_UNLOCK_CONFIRMED') {
            if (window.SantisLog) window.SantisLog.info('🔓 [APEX] Otonom evrim yeniden aktif');
        }
    }
    ...
    renderDarwinianFitness(data) {
        const feed = document.getElementById('darwinian-fitness-feed') || document.getElementById('live-pulse-feed');
        if (!feed) return;

        const variants = data.payload?.topVariants || data.topVariants || [];
        if (variants.length === 0) return;

        // Başlık kartı
        const header = document.createElement('li');
        header.className = 'p-2 rounded border border-emerald-500/30 bg-emerald-500/5 mb-2 text-center';
        header.innerHTML = `
            <div class="text-[8px] text-emerald-400 uppercase tracking-[3px] mb-1">🧬 Darwinian Fitness Report</div>
            <div class="text-[10px] text-gray-400">${variants.length} varyant | ${new Date().toLocaleTimeString()}</div>
        `;
        feed.prepend(header);
        ...
        variants.slice(0, 5).forEach((v, i) => {
            ...
            const barWidth = Math.min(100, (v.fitness / Math.max(variants[0].fitness, 1)) * 100);
            const glowClass = i === 0 ? 'border-[#D4AF37]/50 bg-[#D4AF37]/5' : 'border-gray-800 bg-gray-900/30';
            const lockBtn = i === 0 ? `<button onclick="SovereignRadar.apexLock('${(v.hash || '').replace(/'/g, '')}')" class="ml-2 px-2 py-0.5 bg-[#D4AF37] text-black text-[8px] uppercase tracking-wider rounded hover:bg-white transition-colors">🔒 LOCK APEX</button>` : '';

            item.className = `p-2 rounded border ${glowClass} mb-1 transition-all duration-300`;
            item.innerHTML = `
                <div class="flex items-center justify-between mb-1">
                    <span class="text-gray-300 text-[10px] font-mono">${medal} ${(v.hash || '').substring(0, 25)}...${lockBtn}</span>
                    <span class="text-[#D4AF37] font-bold text-xs">[${v.fitness}]</span>
                </div>
                <div class="w-full bg-gray-800 rounded-full h-1">
                    <div class="bg-gradient-to-r from-emerald-500 to-[#D4AF37] h-1 rounded-full transition-all duration-500" style="width:${barWidth}%"></div>
                </div>
                <div class="flex justify-between mt-1 text-[8px] text-gray-500">
                    <span>${v.clicks} tık</span>
                    <span>${v.dwellSec || 0}s bekleme</span>
                    <span>${v.cards || 0} kart</span>
                </div>
            `;
            feed.prepend(item);
        });
    ...
    renderDarwinianEvent(data) {
        const feed = document.getElementById('darwinian-event-feed') || document.getElementById('live-pulse-feed');
        if (!feed) return;

        const p = data.payload || data;
        const isClick = p.eventType === 'click';
        const icon = isClick ? '🖱️' : '👁️';
        const color = isClick ? 'text-emerald-400' : 'text-blue-400';

        const item = document.createElement('li');
        item.className = 'p-1.5 rounded border border-gray-800 bg-gray-900/30 mb-1 flex items-center justify-between text-[10px] pulse-entry';
        item.innerHTML = `
            <span class="${color}">${icon} ${p.cardId || 'unknown'}</span>
            <span class="text-gray-500 font-mono">${(p.variantHash || '').substring(0, 20)}</span>
            <span class="text-[#D4AF37] font-bold">[${p.fitness || 0}]</span>
        `;
        feed.prepend(item);
    ...
    // 👑 APEX LOCK/UNLOCK — Admin varyant kontrolü
    apexLock(variantHash) {
        if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
            console.warn('🚫 [War Room] WS bağlantısı yok — Apex Lock gönderilemedi');
            return;
        }
        this.socket.send(JSON.stringify({
            channel: 'system',
            action: 'apex_lock',
            payload: { variantHash, timestamp: Date.now() }
        }));
        console.log(`👑 [War Room] APEX LOCK komutu gönderildi: ${variantHash}`);
    }
    ...
    apexUnlock() {
        ...
        console.log('🔓 [War Room] APEX UNLOCK komutu gönderildi');
    }
}
