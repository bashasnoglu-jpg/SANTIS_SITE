...
import { FileBackedEMAOptimizerMemory } from './server/core/experiments/optimizer/optimizer.memory.aggregate.ema.file.ts';
import { TemporalEMAOptimizerAdapter } from './server/core/experiments/optimizer/optimizer.temporal.ema.adapter.ts';
...
async function run(): Promise<void> {
  ...
  const freshDate = new Date(now - 1 * 3_600_000).toISOString(); // 1 saat once
...
run().catch((error) => {
  console.error('❌ V2.6 EMA EXPONENTIAL DECAY SMOKE TEST FAILED');
  console.error(error);
  process.exitCode = 1;
});
