import { OptimizerBanditAdapter } from './server/core/experiments/optimizer/optimizer.bandit.adapter.ts';
...
function run(): void {
  ...
  const promising = ranked.find((x) => x.variantId === 'variant_b');
  const weak = ranked.find((x) => x.variantId === 'variant_c');
