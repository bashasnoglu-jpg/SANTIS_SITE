...
runWithPrivateServerBoundary({
  context: 'Governance Smoke Test',
  requiredPaths: [
    'server/core/concierge/governance/governance.adapter.ts'
  ],
  run: async () => {
    const { deriveGovernanceScoreboard } = await import('./server/core/concierge/governance/governance.adapter.ts');
    ...
    cases.forEach(c => {
      ...
      if (c.expected.attributedAction) {
        const hasAttr = scoreboard.attribution.some((a: any) => a.actionType === c.expected.attributedAction);
        if (!hasAttr) {
          pass = false;
