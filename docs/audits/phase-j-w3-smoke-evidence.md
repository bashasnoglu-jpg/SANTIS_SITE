...
- [x] Open `public/admin/audit-logs.html` in the browser.
- [x] Verify that no network request to `/api/v1/boardroom/audit-log` is made.
- [x] Verify that the screen prominently displays `AUTHENTICATION REQUIRED. TOKEN MISSING.`.
- [x] Verify that the table, pagination, and filter bar are hidden.

### 2. Authorization Injection
- [x] Open the browser Developer Console (F12).
- [x] Execute `AdminAuth.setAuthToken('VALID_JWT_HERE')` using a token with admin/boardroom roles.
- [x] Reload the page.
- [x] Verify that the network request completes with `200 OK`.

### 3. Data Rendering & Pagination
- [x] Verify that the Audit Log table renders the retrieved rows correctly.
- [x] Verify that timestamps, events, actor IDs, and IP addresses map to the correct columns.
- [x] Verify that the pagination indicator shows "Page 1" and the correct total count.
- [x] Click "Next" (if applicable) and verify the table updates correctly with the new offset.

### 4. Filter Bar Behavior
- [x] Change the "Event" filter to `auth.login` and click Search.
- [x] Verify the table reloads and only shows `auth.login` events.
- [x] Clear the filters by clicking "Reset" and verify the full list returns.

### 5. Drawer & XSS Escape Verification
- [x] Click the "Inspect" button on any log entry.
- [x] Verify the side drawer slides out smoothly.
- [x] Verify the ID, Tenant ID, User Agent, and JSON Payload are displayed correctly.
- [x] **Crucial Security Check:** If a payload contains `<script>alert(1)</script>`, verify it is rendered as raw text and NOT executed (HTML Escaping).
- [x] Click the overlay or the "X" button and verify the drawer closes.
