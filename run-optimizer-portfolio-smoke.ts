import { OptimizerPortfolioAdapter } from './server/core/experiments/optimizer/optimizer.portfolio.adapter.ts';
...
function run(): void {
  ...
  const selectedIds = result.selected.map((item) => item.variantId);
