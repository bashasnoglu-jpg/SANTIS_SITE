...
(function () {
    const Engine = {
        ...
        buildBentoCard(item, bentoClass, index) {
            // Updated V5.1 Link Logic (Fixes 404s)
            const urlParam = item.slug ? `product=${item.slug}` : `product=${item.id}`;
            const href = `/tr/urunler/detay.html?${urlParam}`; // Absolute path safe
