...
function createCardElement(product, index, isAdminItem = false, ctx = "") {
    ...
    let detailUrl = product.detailUrl || `/service-detail.html?slug=${slug}`;
    if (!product.detailUrl) {
        if (!product.slug && product.id) detailUrl = `/service-detail.html?id=${product.id}`;
        if (isAdminItem && product.slug) detailUrl = `/service-detail.html?slug=${product.slug}`;
    }
    if (product.match_score) detailUrl += `${detailUrl.includes('?') ? '&' : '?'}score=${product.match_score}`; // Concierge Score
