...
export function init() {
    // 🛡️ DETERMINISTIK DOM BARİYERİ: Canvas var mı diye DOM tam hazır olunca bak
    const _boot = () => {
        const canvas = document.getElementById('santis-god-canvas');
        if (!canvas) return null; // Bu sayfada canvas yok — sessizce çık
    ...
    // 🛡️ DOM-READY DISPATCHER: DOM tam hazırsa hemen çalıştır, değilse bekle
    if (document.readyState === 'complete') return _boot();
