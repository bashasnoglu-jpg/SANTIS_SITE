...
runWithPrivateServerBoundary({
  context: 'Constraint-Aware Bandit Smoke Test',
  requiredPaths: [
    'server/core/experiments/optimizer/optimizer.bandit.constraint-aware.adapter.ts'
  ],
  run: async () => {
    const { ConstraintAwareBanditAdapter } = await import('./server/core/experiments/optimizer/optimizer.bandit.constraint-aware.adapter.ts');
    ...
    const safeWinner = result.ranked.find((item: any) => item.variantId === 'variant_a');
    const sameFamily = result.ranked.find((item: any) => item.variantId === 'variant_b');
    const risky = result.ranked.find((item: any) => item.variantId === 'variant_c');
    const traffic = result.ranked.find((item: any) => item.variantId === 'variant_d');
    const guardrail = result.ranked.find((item: any) => item.variantId === 'variant_e');
