...
import { OptimizerLearningPipeline } from './server/core/experiments/optimizer/optimizer.learning.pipeline.ts';
...
run().catch((error) => {
  console.error('❌ V2.4 LEARNING GUARD SMOKE TEST FAILED');
  console.error(error);
  process.exitCode = 1;
});
