...
class RadarEngine {
    ...
    initDOM() {
        setTimeout(() => {
            ...
            elements.forEach(id => {
                ...
                if (el) { el.classList.remove('opacity-0', 'translate-y-4'); } else { console.warn('RadarEngine: Missing DOM element ' + id); }
            });
        }, 100);
    }
    ...
    initChart() {
        ...
        if (typeof Chart === 'undefined') {
            console.warn('RadarEngine: Chart.js is not loaded. Degrading gracefully.');
            return;
        }
    ...
    initLiveFeed() {
        ...
        this.sseSource.onerror = (err) => {
            console.warn('RadarEngine: SSE Connection error, switching to fallback.', err);
    ...
    async setTimeCursor(timestamp) {
        ...
        } catch (err) { console.warn('RadarEngine: Reconstruction Error:', err); this.exitHistoricalMode(); }
    }
    ...
    async syncWithBackend() {
        try {
            ...
            if (!response.ok || !response.headers.get('content-type')?.includes('application/json')) {
                throw new Error('API not available or returned HTML instead of JSON');
            }
...
document.addEventListener('DOMContentLoaded', () => {
    if (window.RadarEngine) {
        console.warn('RadarEngine already initialized. Skipping duplicate init.');
        return;
    }
