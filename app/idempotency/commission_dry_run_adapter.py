...
class AirtableGetOnlyReader:
    """Minimal Airtable reader with GET-only surface and no write methods."""
    ...
    def get_record(
        ...
        *,
        ...
        try:
            ...
            raise CommissionDryRunAdapterError(
                "AIRTABLE_READ_FAILED", record_id, body[:500]
            ) from exc
        except URLError as exc:
            raise CommissionDryRunAdapterError(
                "AIRTABLE_NETWORK_ERROR", record_id, str(exc.reason)
            ) from exc
        except json.JSONDecodeError as exc:
            raise CommissionDryRunAdapterError(
                "AIRTABLE_INVALID_JSON", record_id, "Airtable returned invalid JSON"
            ) from exc
...
def _single_link(fields: Mapping[str, Any], name: str, *, required: bool) -> str | None:
    ...
    if value in (None, "", []):
        if required:
            raise CommissionDryRunAdapterError(
                "ADAPTER_IDENTITY_INCOMPLETE", name, "required linked record is missing"
            )
        return None
    if not isinstance(value, list) or len(value) != 1 or not isinstance(value[0], str):
        raise CommissionDryRunAdapterError(
            "ADAPTER_CARDINALITY_ERROR", name, "expected exactly one linked record"
        )
    return value[0]
...
def _select_name(value: Any, *, field: str, required: bool = False) -> str | None:
    if value in (None, ""):
        if required:
            raise CommissionDryRunAdapterError(
                "ADAPTER_FIELD_MISSING", field, "required select value is missing"
            )
        return None
    ...
    raise CommissionDryRunAdapterError(
        "ADAPTER_INVALID_SELECT", field, "expected select name"
    )
...
def _decimal_string(value: Any, *, field: str, places: int) -> str | None:
    ...
    try:
        ...
        raise CommissionDryRunAdapterError(
            "ADAPTER_INVALID_DECIMAL", field, "invalid decimal value"
        ) from None
    if not amount.is_finite():
        raise CommissionDryRunAdapterError(
            "ADAPTER_INVALID_DECIMAL", field, "decimal must be finite"
        )
...
def _gross_source_amount(
    ...
    if segment is not None and segment.get("Segment Price EUR") not in (None, ""):
        value = _decimal_string(
            segment.get("Segment Price EUR"), field="Segment Price EUR", places=2
        )
        assert value is not None
        return value
    for field in ("Total_Amount_EUR_New", "Final Amount EUR"):
        if booking.get(field) not in (None, ""):
            value = _decimal_string(booking.get(field), field=field, places=2)
            assert value is not None
            return value
    raise CommissionDryRunAdapterError(
...
def _calculated_commission_amount(
    *,
    ...
    if calculation_type == "Percent":
        ...
        if rate is None:
            raise CommissionDryRunAdapterError(
                "ADAPTER_RATE_MISSING", "Rate Percent", "Percent rule requires Rate Percent"
            )
        ...
        raise CommissionDryRunAdapterError(
            ...
            "Calculation Type",
            f"supports Percent and Fixed Amount only; observed {calculation_type!r}",
        )
...
def build_commission_dry_run_from_airtable(
    ...
    *,
    ...
    """Read exact Airtable records, invoke pure builder, and return NOOP output."""
    ...
    if rule.get("Active") is not True:
        raise CommissionDryRunAdapterError(
            "ADAPTER_RULE_NOT_ACTIVE", "Active", "commission rule must be active"
        )
    ...
    booking_service_id = _single_link(booking, "Service_Link", required=True)
    ...
    effective_service_id = booking_service_id
    ...
    if segment is not None:
        ...
        segment_environment = _select_name(
            segment.get("Environment"), field="Environment", required=True
        )
        segment_service_id = _single_link(segment, "Service_Link", required=True)
        ...
        effective_service_id = segment_service_id

    if rule_service is not None:
        _require_equal("Service_Link", effective_service_id, rule_service)

    target = _select_name(
        rule.get("Commission Target"), field="Commission Target", required=True
    )
    ...
    calculation_type = _select_name(
        rule.get("Calculation Type"), field="Calculation Type", required=True
    )
    ...
    component = _COMPONENT_BY_TARGET[target or ""]
    ...
    state = {
        ...
        "payment_status": _select_name(
            booking.get("Payment_Status_New"), field="Payment_Status_New"
        ),
        "payout_status": None,
