...
async def _god_health_priority():
    """Phase 30: God Mode SHI — Priority route."""
...
async def _slots_health_priority():
    """Phase 28: Slot Radar — Priority route."""
    try:
        from app.db.session import AsyncSessionLocal
        from sqlalchemy import text as _t
        async with AsyncSessionLocal() as _db:
...
async def _simulate_priority(payload: dict = Body(...)):
    """Phase Sandbox: Simulator — Priority route."""
    import math, random
...
ALLOWED_ORIGINS = [
    ...
    "http://127.0.0.1:8000",
    "http://localhost:3000",    # Uvicorn dev / Next.js
    "http://127.0.0.1:3000",
    os.getenv("PRODUCTION_ORIGIN", "https://santis.ai")
]

# ─── SOVEREIGN API GUARD MIDDLEWARE ─────────────────────────────────────────
    ...
    async def dispatch(self, request, call_next):
        ...
        if path == "/api/v1/analytics/god/health" and method == "GET":
            try:
                ...
                occupied = []
            KNOWN = ["hero_home","hero_hamam","hero_masaj","hero_cilt","card_hamam_1","card_hamam_2","card_masaj_1","card_masaj_2","card_cilt_1","highlight_home"]
            empty = [s for s in KNOWN if s not in {r[0] for r in occupied}]
            def w(s): return 1.5 if "hero" in s else 1.2 if "card" in s else 1.0
            num = sum(float(r[1]) * w(r[0]) for r in occupied)
            den = sum(w(r[0]) for r in occupied) + len(empty) * 1.2
            shi = round((num / den if den > 0 else 0) * 100, 1)
            ...
            return _JSONResponse({"shi":shi,"shi_status":"sovereign" if shi>=85 else "elevated" if shi>=70 else "alert",
                "slot_breakdown":{"optimal":sum(1 for r in occupied if r[1]>=0.75),"at_risk":sum(1 for r in occupied if 0.5<=r[1]<0.75),"critical":sum(1 for r in occupied if r[1]<0.5),"empty":len(empty)},
                "est_portfolio_lift":round(sum(float(r[1])*w(r[0])*580 for r in occupied),0),"alerts":alerts[:5],"ts":_mdt.datetime.utcnow().strftime("%H:%M:%S")})
            ...
            try:
                ...
                async with AsyncSessionLocal() as _db:
                    rows = await _db.execute(_t("""
                        SELECT ga.slot, ga.id, ga.url, ga.filename, ga.category,
                               COALESCE(ai.sas_score, 0.0) AS sas_score
                        FROM gallery_assets ga LEFT JOIN asset_intelligence ai ON ai.asset_id = ga.id
                        WHERE ga.slot IS NOT NULL AND ga.slot != '' ORDER BY ga.slot ASC
                    """))
                    occupied = rows.fetchall()
            except Exception:
                occupied = []
            KNOWN = ["hero_home","hero_hamam","hero_masaj","hero_cilt","card_hamam_1","card_hamam_2","card_masaj_1","card_masaj_2","card_cilt_1","card_cilt_2","highlight_home","card_wellness_1","card_wellness_2","hero_galeri","hero_rezervasyon","hero_iletisim","card_kids_1","feature_kids","feature_detox"]
            omap = {}
            for r in occupied:
                sas = float(r[5])
                omap[r[0]] = {"slot":r[0],"asset_id":str(r[1]),"url":r[2] or r[3] or "","category":r[4],"sas_score":round(sas,4),"health":"optimal" if sas>=0.75 else "at_risk" if sas>=0.50 else "critical"}
            return _JSONResponse({"slots":[omap.get(s,{"slot":s,"asset_id":None,"url":None,"category":None,"sas_score":0.0,"health":"empty"}) for s in KNOWN],"total":len(KNOWN)})
            ...
            aes = float(payload.get("aesthetic_threshold", payload.get("conversion_rate", 0.12)))
            conv = min(0.35, aes * 0.45)
            bookings = _mmath.ceil(int(payload.get("sessions", 800)) * conv)
            price = round(base * surge * (1 + aes * 0.3), 2)
            mrr = int(round(bookings * price * 30, 0))
            return _JSONResponse({"predicted_mrr":mrr,"predicted_bookings":bookings,"predicted_occupancy_pct":min(98,round(conv*100*surge,1)),"dynamic_price":price,"projected_revenue":mrr,"bookings_est":bookings,"multiplier_applied":surge,"confidence":round(0.72+_mrand.uniform(-0.05,0.15),2),"status":"SIMULATION_COMPLETE"})
...
app.add_middleware(
    ...
    allow_credentials=True,     # 🚨 Çerezlerin (HttpOnly) API'ye geçmesine izin vermek için ŞARTTIR!
    allow_methods=["*"],
    allow_headers=["*"],
)
...
from starlette.middleware.base import BaseHTTPMiddleware
from app.core.middleware import CorrelationIdMiddleware, GlobalAuditMiddleware
...
# ── Phase 18: Sovereign Tenant Shield & DNA Correlation ────────
from app.core.tenant_shield import SovereignTenantMiddleware
app.add_middleware(SovereignTenantMiddleware)
...
# ─── PHASE 28/30: DIRECT ROUTE OVERRIDES (Router chain bypass) ───────────────────

@app.get("/api/v1/analytics/god/health")
async def god_health_direct(db: AsyncSession = Depends(get_db)):
    """Phase 30: God Mode — SHI direct endpoint."""
    try:
        from sqlalchemy import text as _t
        ...
        weights = {"hero": 1.5, "card": 1.2}
        def w(slot): return next((v for k, v in weights.items() if k in slot), 1.0)
        ...
        alerts = []
        if shi < 85:
            alerts.append({"severity":"WARNING","msg":f"SHI {shi}% below 85%","ts": __import__("datetime").datetime.utcnow().strftime("%H:%M")})
        for s in empty_slots[:3]:
            alerts.append({"severity":"VACANCY","msg":f"Empty slot: '{s}'","ts": __import__("datetime").datetime.utcnow().strftime("%H:%M")})
        if not alerts:
            alerts.append({"severity":"OK","msg":"All systems Sovereign.","ts": __import__("datetime").datetime.utcnow().strftime("%H:%M")})
        return {"shi":shi,"shi_status":"sovereign" if shi>=85 else "elevated" if shi>=70 else "alert",
                "slot_breakdown":{"optimal":optimal,"at_risk":at_risk,"critical":critical,"empty":len(empty_slots)},
                "est_portfolio_lift":est_lift,"alerts":alerts[:5],
                "ts": __import__("datetime").datetime.utcnow().strftime("%H:%M:%S")}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/v1/media/slots/health")
async def slots_health_direct(db: AsyncSession = Depends(get_db)):
    """Phase 28: Slot Radar — direct endpoint."""
    try:
        from sqlalchemy import text as _t
        rows = await db.execute(_t("""
            SELECT ga.slot, ga.id, ga.url, ga.filename, ga.category,
...
@app.post("/api/v1/analytics/simulate")
async def simulate_scenario(payload: dict = Body(...)):
    """Phase Sandbox: Revenue scenario simulator."""
    import math, random
    multiplier = float(payload.get("multiplier", 1.0))
    sessions = int(payload.get("sessions", 100))
    conversion = float(payload.get("conversion_rate", 0.12))
    aov = float(payload.get("aov", 185))
    projected = round(sessions * conversion * aov * multiplier, 2)
    return {"status":"SIMULATION_COMPLETE","projected_revenue":projected,
            "bookings_est":math.ceil(sessions*conversion),"multiplier_applied":multiplier,
