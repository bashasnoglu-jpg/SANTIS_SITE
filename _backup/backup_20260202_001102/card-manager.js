 ...
 * Handles "Quick View" modal logic for standardized cards (.prod-card-v2) across all pages.
 * Requires: assets/css/master-cards.css
...
function openQv(p) {
    ...
    let finalSrc = "";
    if (p.img && (p.img.startsWith('assets') || p.img.startsWith('/') || p.img.startsWith('http'))) {
        finalSrc = p.img.startsWith('/') ? p.img.substring(1) : p.img;
    } else {
